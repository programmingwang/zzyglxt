...
public interface IndustrialDevelopSaleDrugMapper {
    ...
    IndustrialDevelopSaleDrugDto selectByPrimaryKey(@Param("itemid") Integer itemid, @Param("itemcode") String itemcode);

    int updateByPrimaryKeySelective(IndustrialDevelopSaleDrug record);

    int updateByPrimaryKey(IndustrialDevelopSaleDrug record);
/*通过orgCode查询所有售药记录*/
    List<IndustrialDevelopSaleDrugDto> selectAllSaleDrug( @Param("status") String status, @Param("orgCode") String orgCode);
